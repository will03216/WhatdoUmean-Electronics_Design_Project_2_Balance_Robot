...
// angle control: bias is good enough, but don't know if pid is used
//                in principle, could use yawpid, but i didn't try

// lt and oa and others change to cmd control: done
// isolate turning pi/2 codes
// getPosition()

// commented: pitch, speedCmPerSecond, cumulaDis, turnVal, vDesired, speedOutput, Serial.print(balanceOutput);
//  Serial.print(" ");
...
// turn faster
// add precise control
// use precise turning to explore maze first thing in the morning. if not working, use more ultrsensors
// position sensor
// teams-reflections
// yaw correction
// pid line tracking
// complementary filter constant tuning

// move all consaants outside of loop()
// movement video and line tracking video
// Battery usage
...
void loop()
{
    ...
    if (millis() > loopTimer)
    {
        ...
        if (Serial.available() > 0)
        {
            ...
            {
                vDesired = 10 * wheelDiameter / 2; //~33.5cm/s // 30*wheelDiameter/2
                turnVal = 0;
                yawAngle = 0.0;
                targetYawAngle = 0;
                preciseTurning = false;
