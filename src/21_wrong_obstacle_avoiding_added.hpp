...
const float OBSTACLE_DISTANCE_THRESHOLD = 5; // cm
...
void loop()
{
  ...
  if (millis() > loopTimer)
  {
    ...
    if(oaen){
      if (obstacleDistance < OBSTACLE_DISTANCE_THRESHOLD)
    {
    ...
    else if (obstacleDistance >= OBSTACLE_DISTANCE_THRESHOLD) //stay still
    {
  ...
  if (oaen && autoTurn && millis() - turnStartTime >= TURN_DURATION_MS)
  {
