...
PID balancePid(900,0,50,0); //(900,0,50,0) //(9000, 5, 30, 0); //(9000, 17, 80, 0); //adjust
PID speedPid(2.6, 0, 0.9, 0); //(3.6, 0, 0.9, 0) // (1.0, 0.38, 0.23, 0); //adjust 这里 以及 下面的2个
PID yawPid(2.9, 4.0, 0, 0.0);
...
void loop() {
  ...
  if (millis() > loopTimer) {
    ...
    float pitch = atan2(a.acceleration.z, a.acceleration.x ); //- 0.03;//- 1.2*PI/180; //could add bias
    //delay(300);
