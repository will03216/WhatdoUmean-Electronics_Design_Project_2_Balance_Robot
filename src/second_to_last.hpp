...
// solder,screw boards onto body,tidy up
// put battery part into ISR
...
// faster w and s (maybe smaller p)
...
PID speedPid(4.5, 0.0, 0.5, 0.0);        // 4.5 //3.5 falls once //(not a) 90  //2.2-2.6 // smaller p i d values //(3.6, 0, 0.9, 0) // (1.0, 0.38, 0.23, 0)
PID positionPid(2.5, 0.035, 0.0, 0.0);   // 1,0.5,0.7 //6 //10,0,3 is wrong  // must have lowest bandwidth
...
void loop()
{
    ...
    if (millis() > loopTimer)
    {
        ...
        float pitch = atan2(a.acceleration.z, a.acceleration.x) + 0.005; // bias
        // Serial.print(pitch);
        // Serial.println();
        // delay(300);
