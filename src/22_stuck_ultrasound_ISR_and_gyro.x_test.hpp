...
//               in principle, cannot use yawpid

//lt and oa and others change to cmd control
//SOMETIMES FALL
#include <Arduino.h>
#include <SPI.h>
...
PID turnPid(30.0, 0.0, 50.0, 0); //30,0,20 //20,0,0.2 // (14,0,0.2,0)

// === GLOBAL STATE ===

bool turningToAngle = false;
...
void loop()
{
    ...
    if (millis() > loopTimer)
    {
        ...
        if (turningToAngle)
        {
            ...
            float pidTurnOutput = turnPid.compute(yawAngle)/10; // This is the new `turnVal`
        ...
        if (obstacle_avoiding_enable && autoTurn && millis() - turnStartTime >= TURN_DURATION_MS)
        {
